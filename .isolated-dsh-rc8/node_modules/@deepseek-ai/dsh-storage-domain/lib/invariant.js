...
const name = "storage-domain-invariant";
...
const install = Object.assign((ctx, fail) => {
	ctx.on("domain/changed", (change) => {
		...
		if (domain === void 0) return fail(`domain/changed for '${change.domain}' emitted while that domain is not open`);
		if (change.table === "") {
			if (domain.global.get() !== change.value) return fail(`domain/changed global value for '${change.domain}' differs from the in-memory global`);
			return;
		}
		const current = domain.table(change.table).get(change.key);
		switch (change.operation) {
			case "deleted":
				if (current !== void 0) return fail(`domain/changed deletion of '${change.domain}'.'${change.table}'['${change.key}'] emitted while the record is still in memory`);
				return;
			case "put":
