...
export class WorkflowAgent {
  constructor({ taskRepository, projectAgentsRegistry, knowledgeRepository = null, log = () => {} } = {}) {
  ...
  createTasks({ project_id: projectId, type = 'workflow.project_task', goal, brief = {}, subtasks = [], priority = 5, worker_selector = {}, created_by = 'workflow:agent', agent_id = null } = {}) {
    ...
    return entries.slice(0, 32).map((entry, index) => {
      ...
      const created = this.tasks.create({
        ...
        project_id: projectId, agent_id: entry.agent_id ?? agent_id ?? agent.agent_id,
        created_by,
      });
